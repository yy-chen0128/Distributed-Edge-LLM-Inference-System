...
| **elasticity-cost-benefit.md** | **弹性（R2）的代价与收益**：先把 R1（运行期改切分）与 R2（在途请求不丢）分开；四个选项（丢弃/重放/搬 KV/冗余镜像）的代价对比（按 Azure code/conv、Mooncake、Codex 四种真实画像算）；最便宜的恢复＝"把已生成 token 拼回 prompt 重放一次 prefill"，不需要 KV 导出导入、不需要离开机器配合、不需要冗余；分档设计（档0 停-重组-重启 / 档1 重放 / 档1b 搬 KV / 档2 镜像）；结论是第一优先不是恢复而是压缩重配置的装载时间 |
