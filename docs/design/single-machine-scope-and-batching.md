...
| 盈亏平衡 prompt 长度 | **0.5B ≈ 463 token；7B ≈ 27 token** | 固定开销 = 模型计算 的临界点 |

### 3.4 对更大模型的投影（按每层权重大小线性缩放）

Qwen2.5-7B 每层权重 510.1 MB = 0.5B 的 **17.1 倍**（28 层；$4\cdot3584^2+3\cdot3584\cdot18944$ 参数，bf16）。
固定开销 $c_0$ 不变，边际成本 ×17.1：

| prompt 长度 | 0.5B 开销占比 | **7B 开销占比** |
|---|---|---|
| 38 token（我们现在的测试） | 92.4% | **41.6%** |
| 512 token | 47.5% | **5.0%** |
| 4096 token | 10.2% | **0.7%** |
