...
### 3.4 7B 的真实数字（已实测，替换掉早先的错误投影）

早先这里放的是"按每层权重大小等比缩放"的**投影**，结论是"7B 在 38 token 时固定开销只占 43.5%"。
**这个投影是错的，现已实测**（`scripts/probe_7b_stage.py`，同一台 4060 Laptop，7B 只装 4 层）：

| | 0.5B（实测） | **7B（实测）** | 比值 |
|---|---|---|---|
| 每层权重 | 29.8 MB | **466.1 MB** | 15.6× |
| 每次调用固定开销 $c_0$ | 0.97–0.99 ms/层 | **2.55 ms/层** | **2.6×** |
| 每 token 边际成本 $c_1$ | 0.00214 ms/token/层 | **0.0217 ms/token/层** | **10.1×** |
| decode 单 token | 0.85 ms/层 | **2.60 ms/层** | 3.1× |
| decode 相对带宽下界 | 慢 7 倍 | **慢 1.4 倍**（1.86 GB ÷ 256 GB/s = 7.3 ms vs 实测 10.4 ms） | — |

**关键：$c_0$ 只涨了 2.6 倍，$c_1$ 涨了 10.1 倍。** 因为 $c_0$ 是**主机侧**的每次调用开销
（kernel launch + Python dispatch），它随张量变大只略微变贵，而真正的模型计算按权重大小等比增长。
所以"开销占比"**不是**按模型大小线性反转的：

| prompt 长度 | 0.5B 开销占比 | **7B 开销占比（实测标定）** |
|---|---|---|
| 38 token（我们现在的测试） | 92.4% | **75.6%** |
| 512 token | 47.5% | **18.7%** |
| 4096 token | 10.2% | **2.8%** |
| 盈亏平衡长度 | 463 token | **118 token** |

**修正后的结论**：**"框架开销占绝大部分"对 0.5B 和 7B 在 38 token 时都成立**（92% / 76%），
7B 只是"好一些"；要到几百 token 才反转（7B 的平衡点 118 token，0.5B 是 463）。
这**加强了**批处理的必要性——因为 decode 无论多长上下文都是每 token 一次调用，固定开销照付。

（测量细节：7B 中间段 4 层驻留 1864.5 MB、prefill 拟合 `T(P)=10.216+0.08684P`；
首段 4 层驻留 2954.5 MB（含 1.09 GB 词表）、`T(P)=10.232+0.08488P`；
KV 在位置 50 时 409,600 B = 每 token 每层 2048 B，与公式吻合。）

投影是**估算**（假设 7B 在同样的框架、同样的每层效率下运行，且只考虑 dense 权重），
但它足以说明：**"框架开销占绝大部分"是 0.5B + 38 token 这个组合的性质，不是推理系统的性质。**
（⚠️ 这一段的结论已在 §3.4 用实测替换：7B 的实测开销占比是 75.6%，不是投影的 43.5%。）
...
3. **对 7B 的后果（已实测）**：`prepare_epoch` 的**有效速率只有约 53 MB/s**，不是 110 MiB/s——
因为读盘只是一半，另一半是 dtype 转换 + `load_state_dict` 拷贝。
实测：7B 4 层（1864.5 MB）准备 **35.7 s**；首段 4 层含词表（2954.5 MB）**55.5 s**。
按 12/4/4/8 切分，最重的段0（6.7 GB）**≈ 126 s**，每次重配置都要付。这对"节点离开后快速重配置"是致命的。
