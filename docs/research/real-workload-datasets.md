...
- **词表大小决定首/末段的固定占用**：词表对应两个大矩阵——首段的 embedding 与末段的输出头。
  Qwen2.5-7B 词表 152064 × hidden 3584 → fp16 **各 1.09 GB**；Mistral-7B 词表 32768 → 约 **0.23 GB**。
  （0.5B 的实测对照：151936 × 896 → 272.3 MB，与公式吻合。）
- `tie_word_embeddings` 决定末段是否要再背一份输出头：Qwen2.5 的 0.5B/1.5B/3B 绑定，
  **7B/14B 不绑定**（已从下载的 `config.json` 核对）→ 首末段各 1.09 GB，合计 2.18 GB，
  占整个 7B 权重的 14%。**这 2.18 GB 落在哪两台机器上，直接决定四机能否装下。**

### 7.5 词表切分：不要做，改用"把首末段分给大显存机器"

真正的词表切分（把词表矩阵按 vocab 维切开分散到多机）属于**张量并行**，每个 token 都要跨机同步，
与我们已实测的结论冲突（跨 WiFi 做 TP 不可行）。正确做法是在**切层时把词表当固定开销纳入约束**，
把首段/末段固定分给显存大的机器。详见
[`parameter-and-kv-management-status.md`](parameter-and-kv-management-status.md) §8。
